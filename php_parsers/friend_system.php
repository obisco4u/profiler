...
?><?php //this block of code is for when the user either clicks accept or reject
if (isset($_POST['action']) && isset($_POST['reqid']) && isset($_POST['user1'])){// check if variables are set
	...
	if($exist_count[0] < 1){
		mysqli_close($db_conx);
		echo "$user does not exist.";
		exit(); // cheking if requested user exists
	}
	if($_POST['action'] == "accept"){ //acceptance blcok of code
		$sql = "SELECT COUNT(id) FROM friends WHERE user1='$log_username' AND user2='$user' AND accepted='1' LIMIT 1";
		$query = mysqli_query($db_conx, $sql);
		$row_count1 = mysqli_fetch_row($query);
		$sql = "SELECT COUNT(id) FROM friends WHERE user1='$user' AND user2='$log_username' AND accepted='1' LIMIT 1";
	...
	} else if($_POST['action'] == "reject"){ // rejection block of code
